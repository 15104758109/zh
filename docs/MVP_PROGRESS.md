...
- 2026-08-18 `/workbench` 与生产、审计、世界设定、技能设置页面已在 1920×1080、390×844 真实浏览器回归：无横向溢出、无控制台 error；总控书名菜单可打开当前作品范围、从键盘关闭，当前唯一书目整行可选并进入同书工作区，模型配置弹窗遮罩覆盖固定顶栏，模板分类与原生下拉文字可读。总控状态栏只显示业务名称与版本，不显示 Prompt、模板、绑定或作品 UUID/时间戳。流程图核对为 21 个可绑定 LLM 节点、9 个不可绑定关系节点和 1 个参考节点；FP008-03 明确为运行时预检关系节点。全页面静态页测 `502/502`，`pnpm typecheck` 和 Web 静态构建通过。
- 2026-08-18 全页面复核修复了生产页把 `locked_for_deduction` 误画为空内容、总控模型弹窗窄屏布局、共享语义色对比度，以及世界设定、新书、推演、审计的弹窗焦点与背景隔离。当前验收书在 `/production` 真实显示 L1A-005 正在推演并提供既有推演入口；总控真实横幅仍与后端一致：第 10 章、正式资产 14、正式字数 17,910、进度 89.6%、已锁定 L1A / 规划目标为 5 / 4。全页面测试、类型检查和静态构建均通过。
- 书名菜单的跨书切换仍受 `rpc_workbench` 缺少按 `local_operator_id` 返回书目列表读合同限制；当前菜单只展示已验证的当前作品，整行可进入该作品工作区，不伪造其他书籍，属于非工程 P0 的接口缺口。
...
- 2026-08-18 角色页修复了异步正式快照回填后默认展开 L0 抽屉仍保留 max-height: 0 的竞态；当前作品真实快照中的江枫 L0-L3、知识边界和初始记忆均能在页面显示。生产页入口经浏览器实测跳转至 /deduction，完成推演保留在推演页，不自动误跳审计。当前全页面测试为 499/499。
- 2026-08-18 生产页状态遮罩曾因嵌套在内部 flex 容器而从视口 `y=0` 铺满，遮挡顶栏“多代理推演”链接；顶栏还依赖异步 sidebar hydrate 才有 href。现已将该页遮罩限定为 `fixed; inset: 64px 0 0 var(--sidebar-w)`，并由生产页按 `data-book-segment` 直接绑定同书路由；真实浏览器确认遮罩状态下顶栏链接可点击并进入同书 `/deduction`，未修改后端、数据库或 n8n。
- 2026-08-18 审计页真实章节复核清理了原型残留的资产/主编预览语义：正式正文只显示《熔炼末世》服务端返回的江枫章节，不再显示 Elyse/Kaelen 等原型角色；资产视图继续保持禁用并在意外存在时显示“当前审计读取合同未返回叙事资产”，主编提示改为当前正式章节的受控继续/退回语义。角色页与生产页再次实测确认：正式角色只有真实快照中的江枫，L0-L3 与知识边界可见；`locked_for_deduction` 状态提供并实际进入 `/deduction` 的既有入口。
- 2026-08-18 全页面交互复核修复三处前端问题：共享侧栏“审计阶段”改走既有 `/deduction-review` 入口后再补齐章节上下文；多代理推演控制区在窄屏允许按钮换行并保留最小宽度；新书角色卡保留嵌套操作按钮并补齐键盘焦点、Enter/Space 选择。真实浏览器确认侧栏审计入口可恢复当前正式章节，页面测试 `501/501`、类型检查和构建通过。
