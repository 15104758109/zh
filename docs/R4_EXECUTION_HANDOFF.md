...
12. Every n8n implementer prompt names the exact workflow path, immutable start manifest/hash, applicable V7 node-card anchors, Task acceptance, agreed JSON/PostgreSQL contract, RPC names, and test scope. Workflow/Prompt/Code, PostgreSQL/contracts/seeds, and tests may be edited concurrently with disjoint scopes; database application and n8n runtime import remain serialized.
13. A target page must be owned and rendered by `apps/web`. An iframe, redirect, reverse proxy, hard-coded prototype port, runtime read from `docs/前端原型_v2`, or an empty top-level target DOM is `FAIL` even when screenshots match. Verify a fresh target server/build with the prototype server unavailable.

## First Task

Start `WEB-STATIC-RESTORE` as one parent Task. The parent owns routes, App Shell, shared CSS/tokens/components, local assets, and integration. After foundation is stable it may create the three page-group work packages registered in the index. First make all immutable prototypes and targets browser-ready under the recorded external-resource policy, and make the target independently runnable from application-owned files. When all builders are idle and both gates pass, create exactly one independent read-only Codex visual auditor for all nine pages at `1440x900` and `1280x720`; do not use an inline auditor or split the audit by page group. A route/state/console/HTTP pass, an embedded/proxied prototype, or the parent's own screenshot review is insufficient. Any non-`PASS` stays inside the same Task for the single integrated repair and same-auditor recheck.
