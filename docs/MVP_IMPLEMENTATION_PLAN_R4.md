...
5. After all page builders are idle and all eighteen source/target endpoints pass the browser-ready gate, the Orchestrator creates one dedicated read-only `VISUAL_ACCEPTANCE_AUDITOR` as an independent Codex session with the indexed model parameters and a source/role-only `<codex_delegation>` block. It remains an acceptance role inside this parent Task, not a product Task or inline subagent, and owns the complete eighteen-pair verdict matrix.
6. Any `FAIL`, `BLOCKED`, missing comparison, wrong screenshot dimensions, or unverified prototype keeps the Task `IN_PROGRESS`. The parent performs the one integrated repair pass inside this Task, then sends the affected pages back to the same dedicated auditor for recheck.
