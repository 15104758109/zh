...
5. The target must render application-owned top-level DOM and interactions from `apps/web`; it cannot embed, redirect, proxy, or read the prototype directory at runtime. A fresh target dev/build artifact must still work when the prototype baseline server is unavailable.
...
The independent auditor opens the prototype and target route directly and captures its own evidence; coder screenshots, route health, state coverage, and console health cannot substitute for visual comparison. It also inspects the target top-level DOM and network/runtime dependencies, then verifies the target with the prototype server unavailable. Screenshot similarity produced by an iframe, proxy, redirect, or runtime prototype read is an automatic `FAIL`. The prototype and fixed screenshots are the visual facts. The executable page entries in the R4 index define only regions, routes, states, and interactions that screenshots cannot express. Static restore uses `static_mock`; PostgreSQL, n8n, and real model calls are explicitly outside this Task.
