...
Every role below runs in a platform-created independent Codex session with the indexed `gpt-5.6-terra` reasoning parameters. Its `<codex_delegation>` block records only source, role, and Task; actual model identity comes only from platform metadata. Same-window inherited-model inline subagents are not valid delegation for these roles.
...
At Task start, derive a node identity manifest from the workflow blob at the parent Task dispatch HEAD and optionally verify it against a pre-edit local runtime export. A dirty working-tree workflow is a candidate and can never redefine this baseline; record both hashes. The final `(id, name, type)` set must be a superset of the baseline manifest: existing nodes are never deleted, while their prompts, code, parameters, and connections may be updated to satisfy V7. The implementer maps every existing and added node to a V7 node card or Task acceptance and classifies it as `MATCH`, `UPDATE_IN_PLACE`, `ADD_MISSING`, `PRESERVE_UNMAPPED`, or `CONFLICT_CREATOR_REQUIRED`.

Every n8n delegation prompt must name the exact workflow file, its immutable Task-start node manifest and content hash, the applicable V7 node-card headings/anchors, Task acceptance, the agreed Web/JSON/PostgreSQL contract, the PostgreSQL RPC/function names, and the test write scope. This keeps the workflow, data, and test lanes independently executable without allowing any lane to reinterpret the business result. Workflow/Prompt/Code, PostgreSQL/contracts/seeds, and tests/fixtures may be edited concurrently when their write scopes do not overlap; PostgreSQL application and n8n import/activation remain serialized after integration.
...
2. At most three internal page subagents restore their assigned page groups without editing shared files. Each page starts from its indexed prototype file; implementation is a direct UTF-8-preserving migration with only minimal application path, local asset, target route/state, and proven runtime-error replacements. Do not refactor the page DOM, CSS, layout, content hierarchy, or interaction state machine.
...
4. Before audit dispatch, the parent makes every immutable prototype and target browser-loadable without a hanging external dependency. Only recorded local-equivalent asset substitution or runtime URL rewriting is allowed; prototype DOM, layout CSS, content, and interaction logic remain unchanged.
5. The target must render application-owned top-level DOM and interactions from the page's indexed `target_implementation` under `apps/web`; it cannot embed, redirect, proxy, read the prototype directory at runtime, or keep a second route-canonical page copy. A fresh target dev/build artifact must still work when the prototype baseline server is unavailable.
6. After all page builders are idle and all eighteen source/target endpoints pass the browser-ready and target-runtime gates, the Orchestrator creates one dedicated read-only `VISUAL_ACCEPTANCE_AUDITOR` as an independent Codex session with the indexed model parameters and a source/role-only `<codex_delegation>` block. It remains an acceptance role inside this parent Task, not a product Task or inline subagent, and owns the complete eighteen-pair verdict matrix.
7. Any `FAIL`, `BLOCKED`, missing comparison, wrong screenshot dimensions, unverified prototype, empty top-level target DOM, or prototype runtime dependency keeps the Task `IN_PROGRESS`. The parent performs the one integrated repair pass inside this Task, then sends the affected pages back to the same dedicated auditor for recheck.

The independent auditor opens the prototype and target route directly and captures its own evidence; coder screenshots, route health, state coverage, and console health cannot substitute for visual comparison. It also verifies that each route resolves the indexed `target_implementation`, inspects the target top-level DOM and network/runtime dependencies, then verifies the target with the prototype server unavailable. Screenshot similarity produced by an iframe, proxy, redirect, runtime prototype read, or second canonical page copy is an automatic `FAIL`. The prototype and fixed screenshots are the visual facts. The executable page entries in the R4 index define only regions, routes, states, and interactions that screenshots cannot express. Static restore uses `static_mock`; PostgreSQL, n8n, and real model calls are explicitly outside this Task.
